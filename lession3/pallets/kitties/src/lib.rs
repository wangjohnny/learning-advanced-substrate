...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::call_index(2)]
		#[pallet::weight(10_000)]
		pub fn transfer(origin: OriginFor<T>, kitty_id: KittyId, recipient: T::AccountId) -> DispatchResult {
			let who = ensure_signed(origin)?;

			Self::kitty_owner(kitty_id).ok_or::<DispatchError>(Error::<T>::InvalidKittyId.into())?;
